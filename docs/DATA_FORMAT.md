...
가져온 계층은 SQLite의 books, chapters, paragraphs, sentences 테이블에 즉시 저장되며
애플리케이션 재시작 시 다시 불러온다. 장에서 첫 입력이 시작되면 study_sessions에 세션을 만들고,
완료한 문장의 최종 평가 결과는 sentence_attempts에 저장한다. 현재 스키마 버전은 2다.

향후 사용자 작성 퀴즈, 이벤트 시도, 퀴즈 시도 및 복습 통계를 추가로 저장할 수 있어야 한다.
스키마 버전과 마이그레이션은 명시적으로 관리하며, 가져오기나 원문 편집 때문에 기존 학습
이력을 자동 삭제하지 않는다.
